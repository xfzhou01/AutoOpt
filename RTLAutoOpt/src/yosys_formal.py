...
def print_err_info(info_str):
    print("[ERROR] [yosys-formal] -- {}".format(info_str))

def print_warning_info(info_str):
    print("[WARNING] [yosys-formal] -- {}".format(info_str))

def print_info(info_str):
    print("[INFO] [yosys-formal] -- {}".format(info_str))


class yosys_formal_execution:
    ...
    def generate_cmd_str(self, cmd_list:list):
        ...
        Args:
            cmd_list (list): List of commands to be included in 
            the Yosys script.
        """   
        cmd_list = [cmd for cmd in cmd_list if cmd != '']
        yosys_script_cmd_str = "; ".join(cmd_list)
        if self.debug_log_path is not None:
            self.cmd_str = f" yosys -p \"{yosys_script_cmd_str}\""+\
                f" > {self.debug_log_path}"
        else:
            self.cmd_str = f" yosys -p \"{yosys_script_cmd_str}\""+\
                f" > debug_log_yosys_{id(self)}"
    ...
    def execute(self):
        ...
        try:
            ...
            # Execute the command
            print_info(f"Executing yosys command: {self.cmd_str}")
            result = subprocess.run(self.cmd_str, shell=True, 
            ...
            if result.returncode == 0:
                ...
                print_warning_info(f"yosys failed with return code: {result.returncode}. execution time = {execution_time} seconds")
                return result.returncode
        except subprocess.CalledProcessError as e:
            print_warning_info(f"Command failed with error: '{self.cmd_str}'  error is: {e.stderr.decode()} ")
            print_warning_info(f"{self.cmd_str}")
            return -1
